...
use Snow\Payments\Exceptions\UsageDenied;
use Snow\Payments\Services\Period;
use Snow\Payments\Traits\BelongsToPlan;
use Snow\Payments\Traits\HasFeatures;
use Snow\Payments\Traits\HasGracePeriod;
use Snow\Payments\Traits\HasGracePeriodUsage;
use Snow\Payments\Traits\HasPricing;
use Snow\Payments\Traits\HasSubscriptionPeriodUsage;
use Snow\Payments\Traits\HasTrialPeriodUsage;
use Snow\Payments\Traits\HasSchedules;
use Carbon\Carbon;
use DB;
...
class PlanSubscription extends Model
{
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->setTable(config('payments.tables.plan_subscriptions'));
    }
    ...
    public function getRules(): array
    {
        return [
            'tag' => [
                ...
                'alpha_dash',
                'max:150',
                Rule::unique(config('payments.tables.plan_subscriptions'))->where(function ($query) {
    ...
    {
        return $this->hasMany(config('payments.models.plan_subscription_feature'), 'plan_subscription_id', 'id');
    }
    ...
    public function usage(): \Illuminate\Database\Eloquent\Relations\HasManyThrough
    {
        return $this->hasManyThrough(
            config('payments.models.plan_subscription_usage'),
            config('payments.models.plan_subscription_feature'),
    ...
    {
        if (!$ignoreFallback && config('payments.fallback_plan_tag')) { // Do not cancel if a fallback plan is set
            $plan = Plan::getByTag(config('payments.fallback_plan_tag'));
            if (!$plan) {
                throw new UnexpectedValueException('Fallback plan ' . config('payments.fallback_plan_tag') . ' does not exist.');
            }
     ...
     * @param int $uses
     *
     * @return \Snow\Payments\Models\PlanSubscriptionUsage|null
     */
