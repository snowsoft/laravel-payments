...
use Snow\Payments\Exceptions\DuplicateException;
use Snow\Payments\Exceptions\InvalidPlanSubscription;
use Snow\Payments\Models\Plan;
use Snow\Payments\Models\PlanCombination;
use Snow\Payments\Models\PlanSubscription;
use Snow\Payments\Services\SubscriptionPeriod;
use Carbon\Carbon;
...
trait HasSubscriptions
{
    ...
    {
        return $this->morphMany(config('payments.models.plan_subscription'), 'subscriber', 'subscriber_type', 'subscriber_id');
    }
    ...
    public function subscription(?string $subscriptionTag = null)
    {
        ...
        $subscriptionTag = $subscriptionTag ?? config('payments.main_subscription_tag');
     ...
     *
     * @return \Snow\Payments\Models\PlanSubscription|null
     */
    public function subscribedPlans()
    {
        $planIds = $this->subscriptions->reject->isInactive()->pluck('plan_id')->unique();

        return app(config('payments.models.plan'))->whereIn('id', $planIds)->get();
    }
     ...
     * @param \Snow\Payments\Models\Plan|\Snow\Payments\Models\PlanCombination $planCombination Plan pricing and invoice data
    ...
    {
        $tag = $tag ?? config('payments.main_subscription_tag');
